...
bot.on("callback_query:data", async (ctx) => {
  try {
    const callbackData = JSON.parse(ctx.callbackQuery.data);  // Преобразуем данные
    ...
    await ctx.answerCallbackQuery({ text: "Произошла ошибка при обработке вашего ответа." });
  }
});
